...
class Command(BaseCommand):
    ...
    def create_images(self, document_ids=None):
        ...
        for old_image in missing:
            image = DocumentImage(
                ...
                image=f'{old_image.filename}.{old_image.fileformat}',
    ...
    def update_images(self, document_ids=None):
        source_of_truth = OldDocumentImage.objects.annotate(
            filename_with_ext=Concat('filename', Value('.'), 'fileformat')
        ).filter(
            document_id=OuterRef('document_id'),
        ...
        self.debug(
            ...
            '\n'.join(
                f'old: {i.image} new: {i.new_filename}' for i in outdated
            ),
        )
