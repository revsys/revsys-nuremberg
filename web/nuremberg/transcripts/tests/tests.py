...
def seq():
    def get_seq(seq=1):
        ...
        return go_to(
            url_with_query(
                'transcripts:show', transcript.id, transcript.slug(), seq=seq
            )
        )
...
def test_transcript_viewer(seq):
    ...
    assert (
        'This is a corrected copy of transcript for 5 December 1946.'
        in page('p').text()
    )
...
def test_transcript_joins(seq):
    ...
    assert (
        "Wehrmacht, the Medical Service of the SS" in text
    )  # This page break moves to:
    ...
    assert (
        " probably very relevant and important ones.\nHLSL Seq. No. 10" in text
    )
...
def test_evidence_links(seq):
    ...
    page = follow_link(
        page('.document-row')
        .with_text('Transcript for NMT 1: Medical Case')
        .find('a')
    )
...
def test_xml_import():
    ...
    assert transcript_page.page_number == 121
    assert (
        'The defendants Karl Brandt, Genzken, Gebhardt, Rudolf Brandt'
        in transcript_page.text()
    )
